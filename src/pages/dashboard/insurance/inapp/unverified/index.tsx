...
const UnverifiedInsuranceProviders = () => {
  const navigate = useNavigate();
  const { data, isLoading } = useFetch<{ data: Pagination<InsuranceProvider> }>(
    "insurance_provider/unverified-insurance-provider/",
    {
      hideToast: "success",
    }
  );
  ...
  return (
    <section>
      <div className="mb-8">
        <DashboardTitle
          title="Unverified Insurance Providers"
          // action={{}}
        />
      </div>
      <DataTable
        ...
        onRowClick={(row) => navigate(`/app/insurance/${row.id}`)}
      />
    </section>
  );
};

export default UnverifiedInsuranceProviders;
