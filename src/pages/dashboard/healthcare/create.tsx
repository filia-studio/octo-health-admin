...
import type { Healthcare, HealthcareService } from "@/types/healthcare";
...
const CreateHealthcare = () => {
  ...
  const { isLoading, isSuccess } = useFetch<{ data: HealthcareService[] }>(
    "/services/",
    {
      useAuth: false,
      hideToast: "success",
  ...
  const { isLoading: isLoadingHealthcare, data: healthcare } = useFetch<Healthcare>(
    `/healthcare/${id}`,
    {
      useAuth: false,
      enabled: isEdit && isSuccess,
      hideToast: "success",
      onSuccess: (data) => {
        form.reset({
          ...data,
          services: data.healthcare_services.map((service) => ({
            value: services.find((s) => s.label === service)?.value ?? "",
            label: service,
          })),
        });
      },
  ...
  const { isPending: isCreating, mutate: createHealthcare } = useSend(
    ...
    {
      useAuth: false,
      onSuccess: () => navigate("/app/healthcare"),
    }
  );

  const { isPending: isUpdating, mutate: updateHealthcare } = useSend(
    `/healthcare/${id}/`,
    {
      useAuth: false,
      method: "put",
  ...
  useEffect(() => {
    if (isEdit && healthcare && services.length) {
      const prevServices = healthcare.healthcare_services.map((service) => ({
        value: services.find((s) => s.label === service)?.value ?? "",
        label: service,
      }));
      form.setValue("services", prevServices);
    }
  }, [services, isEdit, healthcare, form]);

  const loading = isLoading || isLoadingHealthcare || isCreating || isUpdating;

  return (
    <DashboardDetailLayout
      title={isEdit ? "Edit Healthcare" : "Create Healthcare"}
    >
      <Form {...form}>
        <form
